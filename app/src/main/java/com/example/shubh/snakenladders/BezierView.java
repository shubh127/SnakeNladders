...
public class BezierView extends View {
    ...
    private static final int TEXT_SIZE = 0;    // 文字画笔尺寸
    private static final int TEXT_HEIGHT = 0;  // 文本高度
    ...
    private  ArrayList<PointF> mParray;
    ...
    public BezierView(MainActivity context, ArrayList<PointF> pArray) {
        super(context);
        mParray = pArray;
        init();
...
//        mControlPoints = new ArrayList<>(MAX_COUNT + 1);
       ...
       /* mControlPoints.add(new PointF(w / 5, w / 5));
        ...
        mMovingPaint.setColor(Color.BLUE);
    ...
    /*    HashMap<Integer, Ladder> mLadderHash = new HashMap<>();
         HashMap<Integer, Integer> mSnakeHash = new HashMap<>();
        SnakeNLadderBoard boardView = new SnakeNLadderBoard(getContext(), mLadderHash, mSnakeHash);

        ArrayList<PointF> pArray = new ArrayList<>();

        pArray.add(new PointF(boardView.getX(18),boardView.getY(18)));

        pArray.add(new PointF(boardView.getX(36),boardView.getY(36)));

        pArray.add(new PointF(boardView.getX(67),boardView.getY(67)));*/
    ...
    protected void onDraw(Canvas canvas) {
        if (isRunning() && !isTouchable()) {
            ...
            mBezierPaint.setStrokeWidth(CONTROL_RADIUS-5);
            canvas.drawPath(mBezierPath, mBezierPaint);
            // Bezier曲线起始移动点
            mMovingPaint.setColor(Color.parseColor("#ff0000"));
            canvas.drawCircle(mBezierPoint.x, mBezierPoint.y, CONTROL_RADIUS, mMovingPaint);
            mMovingPaint.setStrokeWidth(5);
            canvas.drawLine(mBezierPoint.x, mBezierPoint.y+5, mBezierPoint.x-10, mBezierPoint.y-25,mMovingPaint);
            canvas.drawLine(mBezierPoint.x, mBezierPoint.y+5, mBezierPoint.x+10, mBezierPoint.y-25,mMovingPaint);
